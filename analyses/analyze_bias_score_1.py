...
def main(args):
    ...
    for inst_no in range(args.instruction_k):
        ...
        # FILE READ 부터 맞추기...
        # file_idx 개념이 아니라 inst# persona이름 맞춰서 체크
        for p_no, persona in enumerate(persona_list):
            ...
            def read_file(result_dir, persona, inst_no, target_category):
                ...
                file_path = os.path.join(result_dir, f_template.format(persona, inst_no, target_category))
                try:
                    f_name = glob.glob(file_path)[0]
                except:
                    print("wrong path: {}".format(file_path))
                print(f_name)
            ...
            data = read_file(result_dir, persona, inst_no, target_category)
